...
# Each case is a fused, segmented batched gate/up LoRA-B GEMM:
#   input_x:        (num_tokens, 2 * max_rank)     -- packed gate/up LoRA-A output
#   gate_up_lora_b: (num_loras, 2 * output_dim, max_rank)
#   output:         (num_tokens, 2 * output_dim)
# Unlike QKV, gate and up share the same output width N (= output_dim) and rank K,
# so the output columns are the uniform partition [0, N, 2N] (no output_offset
# argument is required). Cases with "use_base_output": True fuse a residual:
...
# Copied from sglang/kernels/ops/gemm/gate_up_lora_b.py, taking the logic for the
...
def _gate_up_lora_b_kernel(
    ...
    output,
    K,  # max_rank
    output_dim,  # per-projection output width N
    x_stride_0,
    x_stride_1,
    w_stride_0,
    w_stride_1,
    ...
    # gate_up_id decides which of gate,up to compute (0: gate, 1: up).
...
def _band_dims(output_dim: int) -> List[int]:
    """Per-projection output dims [N, N] for gate and up."""
...
def _compute_flops_by_segment(
    ...
    """Effective GEMM flops: sum over segments and gate/up of 2 * M_s * N_p * rank_s."""
...
def _make_inputs(
    ...
    # input_x is the packed gate/up LoRA-A projection: (num_tokens, 2 * max_rank).
    input_x = torch.randn(num_tokens, 2 * max_rank, dtype=dtype, device=device)
    gate_up_lora_b = torch.randn(
        num_loras, n_total, max_rank, dtype=dtype, device=device
    )
    ...
    return {
        ...
        "band_dims": _band_dims(output_dim),
        "n_total": n_total,
    }
...
def _run_cutlass_once(args: Dict[str, Any]):
    ...
    return gate_up_lora_b_fwd(
        ...
        gate_up_lora_b=args["gate_up_lora_b"],
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        ylabel="TFLOP/s",
        plot_name="gate-up-lora-b-fwd-cutlass-vs-triton",
        args={},
    )
)
...
def print_summary(title: str = "Gate/Up LoRA-B Forward Benchmark Results"):
    print("\n" + "=" * 120)
    print(title)
    print("=" * 120)
...
def _parse_args():
    parser = argparse.ArgumentParser(description="Benchmark gate_up_lora_b_fwd on XPU")
    parser.add_argument(
        "--seed",
        type=int,
        default=42,
