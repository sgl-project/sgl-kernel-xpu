...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        line_names=["AOT (sgl_kernel)", "JIT (sglang)"] if HAS_AOT else ["JIT (sglang)"],
        styles=[("blue", "-"), ("green", "-")],
        ylabel="Time (us)",
...
def benchmark(batch_size, head_dim, num_heads, provider):
    ...
    positions = torch.arange(batch_size, dtype=torch.int64, device=device)  # AOT kernel needs int64
    ...
    cos_sin_cache_jit = create_cos_sin_cache(head_dim).to(device=device, dtype=torch.float32)
    ...
    if provider == "aot":
        ...
        def fn():
            apply_rope_inplace(
                q, k, cos_sin_cache_jit, positions,
                is_neox=is_neox,
                rope_dim=head_dim
            )
...
if __name__ == "__main__":
    ...
    print("AOT: sgl_kernel.apply_rope_with_cos_sin_cache_inplace (compiled SYCL kernels)")
