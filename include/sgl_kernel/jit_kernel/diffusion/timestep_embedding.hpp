...
class TimestepEmbeddingKernel {
 public:
  TimestepEmbeddingKernel(
      const TIn* t_ptr, float* output_ptr, int dim, float neg_log_max_period, float scale, int batch_size)
      : t_ptr_(t_ptr),
...
void timestep_embedding_launcher(
  ...
  const int num_threads_per_row = std::max(1, std::min(kMaxThreadsPerBlock, half_dim));
  ...
  queue.submit([&](::sycl::handler& cgh) {
    if (flip_sin_to_cos) {
      cgh.parallel_for(
          ::sycl::nd_range<1>(::sycl::range<1>(num_groups * threads_per_group), ::sycl::range<1>(threads_per_group)),
          TimestepEmbeddingKernel<TIn, true>(t_ptr, output_ptr, dim, neg_log_max_period, scale, batch_size));
    } else {
      cgh.parallel_for(
          ::sycl::nd_range<1>(::sycl::range<1>(num_groups * threads_per_group), ::sycl::range<1>(threads_per_group)),
          TimestepEmbeddingKernel<TIn, false>(t_ptr, output_ptr, dim, neg_log_max_period, scale, batch_size));
    }
  });
}
...
#define DEFINE_TIMESTEP_EMBEDDING_FORWARD(DTYPE_SUFFIX, DTYPE)                                        \
  extern "C" void timestep_embedding_forward_##DTYPE_SUFFIX(                                          \
    ...
    timestep_embedding_launcher<DTYPE>(                                                               \
        queue, t, output, dim, flip_sin_to_cos, downscale_freq_shift, scale, max_period, batch_size); \
  }
