...
constexpr int get_vec_size() {
  ...
  constexpr int base_vec = (kHiddenSize % 8 == 0) ? 8 : (kHiddenSize % 4 == 0) ? 4 : (kHiddenSize % 2 == 0) ? 2 : 1;
...
void rmsnorm_launcher(
  ...
  queue.submit([&](::sycl::handler& cgh) {
    if constexpr (use_vectorized) {
      cgh.parallel_for(
          ::sycl::nd_range<1>(::sycl::range<1>(num_blocks * kThreadsPerBlock), ::sycl::range<1>(kThreadsPerBlock)),
          RMSNormKernel<DType, kHiddenSize>(
              input_ptr, weight_ptr, output_ptr, input_stride, output_stride, static_cast<uint32_t>(num_tokens), eps));
    } else {
      cgh.parallel_for(
          ::sycl::nd_range<1>(::sycl::range<1>(num_blocks * kThreadsPerBlock), ::sycl::range<1>(kThreadsPerBlock)),
...
#define _DEFINE_RMSNORM_FORWARD(DTYPE_SUFFIX, DTYPE, HIDDEN_SIZE)                                                     \
  extern "C" void rmsnorm_forward_##DTYPE_SUFFIX##_##HIDDEN_SIZE(                                                     \
    ...
    rmsnorm_launcher<HIDDEN_SIZE, DTYPE>(queue, input, weight, output, num_tokens, input_stride, output_stride, eps); \
  }
