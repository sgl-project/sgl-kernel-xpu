 ...
 public:
  ...
  void operator()(::sycl::nd_item<1> item) const {
    ...
    for (auto idx = start_worker_id; idx < num_works; idx += num_workers) {
      ...
      DType* input = load_q ? params_.q_ptr + token_id * params_.q_stride + head_id * params_.head_stride
...
class FusedRopeStoreKernel {
 public:
  ...
  // Use ceiling division so every element is covered for non-power-of-2 rope dims
  // (e.g. rope_dim=80 or 96).  Out-of-bound lane indices are guarded below.
  static constexpr int64_t kLanesPerVecGroup = kWorkThreads * (1 + kIsNeox);
  static constexpr int64_t kTotalVecs = kRopeDim / 2;
  static constexpr int64_t kVecSize = (kTotalVecs + kLanesPerVecGroup - 1) / kLanesPerVecGroup;
  static constexpr int64_t kDimPerThread = kVecSize * 2 * (1 + kIsNeox);
  ...
  void operator()(::sycl::nd_item<1> item) const {
    ...
    for (; idx < num_works; idx += num_workers) {
      ...
      DType* input = load_q ? base.q_ptr + token_id * base.q_stride + head_id * base.head_stride
...
void fused_rope_launcher(
  ...
  queue.submit([&](::sycl::handler& cgh) {
    cgh.parallel_for(
        ::sycl::nd_range<1>(::sycl::range<1>(num_blocks * kBlockSize), ::sycl::range<1>(kBlockSize)),
...
void fused_rope_store_launcher(
  ...
  FusedRopeStoreParams<DType, IdType> params{base_params, v_ptr, k_cache, v_cache, out_loc, v_stride, cache_stride};
