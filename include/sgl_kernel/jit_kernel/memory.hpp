...
class SharedMemory {
 public:
  ...
  SharedMemory(const ::sycl::nd_item<3>& item, accessor_t acc) : item_(item), acc_(acc) {}
...
struct TileMemory {
  ...
  static ::sycl::vec<T, VecSize> load_tile(const T* ptr, const ::sycl::nd_item<3>& item) {
   ...
   */
  template <int VecSize>
  static void store_tile(T* ptr, const ::sycl::vec<T, VecSize>& data, const ::sycl::nd_item<3>& item) {
    size_t tid = threadIdx<0>(item);
