...
// [E, N, K/2] with two E2M1 nibbles per byte (low nibble = smaller-K
// element). `scales` is float32 direct-multiplier, laid out
// [E, K/32, N] (K-outer, N-contiguous) so each k-tile's SG scale read
// lands in a single cache line.
