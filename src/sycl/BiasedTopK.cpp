...
struct BiasedTopkKernel : public __SYCL_KER_CONFIG_CONVENTION__ {
  ...
  [[sycl::reqd_sub_group_size(kWarpSize)]] void operator()(sycl::nd_item<3> item) const {
    ...
    for (uint32_t e = lane_id; e < num_experts_; e += kWarpSize) {
      float input_val = static_cast<float>(input_[static_cast<int64_t>(row_idx) * num_experts_ + e]);
      float bias_val = static_cast<float>(bias_[e]);
...
template <typename T, ScoringFunc kScoringFunc>
void launch_biased_topk_kernel(
  ...
  const uint32_t warps_per_token = std::min<uint32_t>((num_experts + kWarpSize - 1) / kWarpSize, 16);
  const bool use_small_token_kernel = num_rows <= kSmallTokenThreshold;
  const uint32_t warps_per_cta = use_small_token_kernel ? warps_per_token : kWarpsPerCTA;
  ...
  auto* bias_ptr = reinterpret_cast<const T*>(bias.data_ptr());
...
void biased_topk(
  ...
  TORCH_CHECK(input.scalar_type() == bias.scalar_type(), "input and bias must have same dtype");
  ...
  TORCH_CHECK(output.device() == input.device(), "output must be on the same device as input");
  TORCH_CHECK(indices.device() == input.device(), "indices must be on the same device as input");
  ...
  SYCL_DISPATCH_FLOATING_TYPES_AND2(
      at::ScalarType::BFloat16, at::ScalarType::Half, input.scalar_type(), "biased_topk", [&]() {
        if (scoring_func == static_cast<int64_t>(ScoringFunc::kSigmoid)) {
          launch_biased_topk_kernel<scalar_t, ScoringFunc::kSigmoid>(
              input,
              bias,
              output,
              indices,
          ...
          launch_biased_topk_kernel<scalar_t, ScoringFunc::kSqrtSoftplus>(
              input,
              bias,
              output,
              indices,
