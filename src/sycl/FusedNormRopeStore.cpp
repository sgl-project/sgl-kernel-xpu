...
inline void local_reduce_sum_sg16(
    float* red, sycl::nd_item<1> item, sycl::sub_group sg, uint32_t tid, float local_val, uint32_t num_sg) {
  ...
  if (sg_id == 0) {
    const float block_sum = subgroup_xor_reduce_sum_16(sg, (lane < num_sg) ? red[lane] : 0.0f);
    if (lane == 0) {
      red[0] = block_sum;
    }
  }
...
struct FusedNormRopeIndexerKernel {
  ...
  void operator()(sycl::nd_item<1> item) const {
    ...
    const auto ps = read_plan_slot(plan_d, plan_c, out_loc, group_id, compress_ratio, is_decode);
    ...
    local_reduce_sum_sg16(red, item, sg, tid, local_sum, kIndexerLocalSize / 16u);
...
struct FusedNormRopeFlashMLAKernel {
  ...
  void operator()(sycl::nd_item<1> item) const {
    ...
    const uint32_t lane = tid & 0xFu;
    const uint32_t sg_id = tid >> 4;
    local_reduce_sum_sg16(red, item, sg, tid, local_sum, kFlashMLALocalSize / 16u);
