...
struct HCSplitSinkhornKernel {
  ...
  [[sycl::reqd_sub_group_size(HC2)]] void operator()(
      sycl::nd_item<1> item) const {
    sycl::sub_group sg = item.get_sub_group();

    constexpr int tokens_per_wg = WG_SIZE / HC2;
    const int token_id = static_cast<int>(item.get_group(0)) * tokens_per_wg
                       + static_cast<int>(sg.get_group_id()[0]);
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> hc_split_sinkhorn(
  ...
  TORCH_CHECK(static_cast<int>(hc_mult) == HC,
      "hc_mult must be ", HC, ", got ", hc_mult);
  TORCH_CHECK(static_cast<int>(sinkhorn_iters) == SINKHORN_ITERS,
      "sinkhorn_iters must be ", SINKHORN_ITERS, ", got ", sinkhorn_iters);
  TORCH_CHECK(mixes.size(-1) == col_size,
      "mixes last dim must be (2+HC)*HC=", col_size, ", got ", mixes.size(-1));
  TORCH_CHECK(hc_scale.numel() == 3, "hc_scale must have 3 elements");
  TORCH_CHECK(hc_base.numel() == col_size,
      "hc_base must have (2+HC)*HC=", col_size, " elements");
