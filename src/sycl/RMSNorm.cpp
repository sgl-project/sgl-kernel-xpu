...
class RMSNormNoRstdForward {
 public:
  using accscalar_t = acc_type<scalar_t>;
  RMSNormNoRstdForward() = delete;
  RMSNormNoRstdForward(
      scalar_t* X_data,
      scalar_t* Y_data,
      weight_t* gamma_data,
      accscalar_t eps)
      : X_data(X_data), Y_data(Y_data), gamma_data(gamma_data), eps(eps) {}

  int get_update_vec_size(int Plane, int vec_size) const {
    vec_size = get_min_vec_size(vec_size, X_data, Y_data, gamma_data);
...
#pragma unroll
      for (int it = 0; it < ITERS; ++it) {
        ...
        if (plane_offset < cfg.Plane) {
          vec_t x_val = *(reinterpret_cast<vec_t*>(X_data + x_group_offset + plane_offset));
          reg[it] = x_val;
#pragma unroll
          for (int v = 0; v < vec_size; ++v) {
...
#pragma unroll
  ...
  accscalar_t reduce_project(sycl::nd_item<1> item_id, const NormConfig& cfg, accscalar_t sum_value) const {
    ...
    return Numerics<accscalar_t>::rsqrt(
        sum_value / static_cast<accscalar_t>(cfg.Plane) + static_cast<accscalar_t>(eps));
  }
...
#pragma unroll
          ...
          *(reinterpret_cast<vec_t*>(Y_data + y_group_offset + plane_offset)) = y_val;
        }
      }
    } else {
      for (index_t plane_offset = lid * vec_size; plane_offset < cfg.Plane;
...
class AddRMSNormNoRstdForward : public RMSNormNoRstdForward<scalar_t, weight_t> {
 public:
  ...
  int get_update_vec_size(int Plane, int vec_size) const {
    vec_size = get_min_vec_size(vec_size, RNF::X_data, RNF::Y_data, RNF::gamma_data, add_data);
    while (Plane % vec_size != 0) {
      vec_size = vec_size >> 1;
    }
    return vec_size;
...
#pragma unroll
          for (int v = 0; v < vec_size; ++v) {
            x_val[v] = static_cast<scalar_t>(static_cast<accscalar_t>(x_val[v]) +
                                             static_cast<accscalar_t>(add_val[v]));
          }
...
#pragma unroll
        ...
        *(reinterpret_cast<vec_t*>(add_data + x_group_offset + plane_offset)) = x_val;
        *(reinterpret_cast<vec_t*>(RNF::X_data + x_group_offset + plane_offset)) = x_val;
#pragma unroll
        for (int v = 0; v < vec_size; ++v) {
          const accscalar_t x = static_cast<accscalar_t>(x_val[v]);
          sum_value += x * x;
...
template <typename scalar_t, typename weight_t, typename Base>
class GemmaRMSNormNoRstdForwardImpl : public Base {
 public:
  using accscalar_t = acc_type<scalar_t>;
  typedef Base RNF;
  GemmaRMSNormNoRstdForwardImpl() = delete;
  using RNF::RNF;
...
template <typename scalar_t, typename weight_t>
using GemmaRMSNormNoRstdForward =
    GemmaRMSNormNoRstdForwardImpl<scalar_t, weight_t, RMSNormNoRstdForward<scalar_t, weight_t>>;

template <typename scalar_t, typename weight_t>
using GemmaAddRMSNormNoRstdForward =
    GemmaRMSNormNoRstdForwardImpl<scalar_t, weight_t, AddRMSNormNoRstdForward<scalar_t, weight_t>>;
...
struct RMSNormNoRstdKernelFunctor {
  ...
  [[sycl::reqd_sub_group_size(NUM_REDUCE_STAGES)]] void operator()(sycl::nd_item<1> item_id) const {
    ...
    const index_t x_group_offset = (row / cfg.input_inner_size) * cfg.input_batch_stride +
                                   (row % cfg.input_inner_size) * cfg.input_inner_stride;
...
template <
    ...
    int vec_size,
    typename Norm>
void dispatch_rmsnorm_no_rstd_iters(
    int iters,
    Norm& norm,
...
void launch_vectorized_rmsnorm_no_rstd_kernel(Norm& norm, const NormConfig& config) {
  // Reset max_vec_size to hardware maximum for wider vector
  // loads, then shrink workgroup_size.
  NormConfig cfg = config;
  cfg.max_vec_size = static_cast<int>(sizeof(float) * 4) / cfg.element_size_bytes;
  int vec_size = norm.get_update_vec_size(cfg.WGPlane, cfg.max_vec_size);
  int workgroup_size = ((cfg.Plane + vec_size - 1) / vec_size + NUM_REDUCE_STAGES - 1) /
                       NUM_REDUCE_STAGES * NUM_REDUCE_STAGES;
  workgroup_size = std::max(workgroup_size, NUM_REDUCE_STAGES);
  if (workgroup_size < cfg.workgroup_size) {
    cfg.workgroup_size = workgroup_size;
    cfg.sub_group_num = workgroup_size / NUM_REDUCE_STAGES;
  }
  const int iters = (cfg.WGPlane + cfg.workgroup_size * vec_size - 1) / (cfg.workgroup_size * vec_size);

#define DISPATCH_RMSNORM_NO_RSTD_VEC(VEC_SIZE) \
  dispatch_rmsnorm_no_rstd_iters<scalar_t, weight_t, VEC_SIZE>(iters, norm, cfg)
...
void GemmaRMSNormKernelImplInternal(
  ...
  auto config = NormConfig(
      ...
      output_inner_size,
      output_inner_stride);
  GemmaRMSNormNoRstdForward<scalar_t, weight_t> gemma_rms_norm_no_rstd_forward(
      X_data, Y_data, gemma_data, eps);
  config.workgroup_num_foreach = 1;
  config.WGPlane = config.Plane;
