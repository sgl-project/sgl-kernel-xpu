...
struct chunk_causal_conv1d_kernel {
 public:
  ...
  static inline sycl::nd_range<2> get_nd_range(
      ...
      const int num_v_heads,
      const int head_v_dim) {
    const int group_size = (2 * head_k_dim + head_v_dim * num_v_heads / num_k_heads) / elems_per_item;
  ...
  static inline int
  get_norm_slm_bytes(const int head_k_dim, const int num_v_heads, const int num_k_heads, const int head_v_dim) {
    const int group_size = (2 * head_k_dim + head_v_dim * num_v_heads / num_k_heads) / elems_per_item;
  ...
  [[sycl::reqd_sub_group_size(sub_group_size)]] void operator()(sycl::nd_item<2> item) const {
    ...
    if (qkv_dim_offset < q_dim) {
      ...
      is_k = true;
      if constexpr (ReorderInput) {
        qkvz_elems_offset = num_k_heads * head_k_dim + k_head_id * k_dim + qkv_dim_offset - (q_dim);
      }
    } else {
      is_v = true;
      if constexpr (ReorderInput) {
        qkvz_elems_offset = 2 * num_k_heads * head_k_dim + k_head_id * v_dim + qkv_dim_offset - (q_dim + k_dim);
      }
    }
    ...
    if (is_q) {
      ...
      reordered_elems_offset = num_k_heads * q_dim + k_head_id * k_dim + qkv_dim_offset - q_dim;
    } else if (is_v) {
      reordered_elems_offset = num_k_heads * (q_dim + k_dim) + k_head_id * v_dim + qkv_dim_offset - (q_dim + k_dim);
    }
...
#pragma unroll
      for (int e = 0; e < elems_per_item; ++e) {
        local_weights[Width * e + i] = conv_weights[(reordered_elems_offset + e) * Width + i];
      }
    }
...
#pragma unroll
      for (int e = 0; e < elems_per_item; ++e) {
        local_input[Width * e + states_load_len + i] = mixed_qkvz[src_token * qkvz_elems + qkvz_elems_offset + e];
      }
    }
...
#pragma unroll
          for (int e = 0; e < elems_per_item; ++e) {
            conv_states_tmp[batch_id * (Width - 1) * conv_elems + i * conv_elems + reordered_elems_offset + e] =
...
#pragma unroll
      for (int e = 0; e < elems_per_item; ++e) {
        q_out[out_token_id * num_k_heads * q_dim + k_head_id * q_dim + qkv_dim_offset + e] = res[e];
      }
    } else if (is_k) {
#pragma unroll
      for (int e = 0; e < elems_per_item; ++e) {
        k_out[out_token_id * num_k_heads * k_dim + k_head_id * k_dim + qkv_dim_offset - q_dim + e] = res[e];
      }
    } else if (is_v) {
#pragma unroll
      for (int e = 0; e < elems_per_item; ++e) {
        v_out[out_token_id * num_k_heads * v_dim + k_head_id * v_dim + qkv_dim_offset - (q_dim + k_dim) + e] = res[e];
      }
    }
  }
...
struct chunk_reorder_zba_kernel {
 public:
  ...
  static inline sycl::nd_range<2> get_nd_range(const int total_seqlen, const int num_k_heads, const int z_dim) {
...
#pragma unroll
        for (int e = 0; e < kv_ratio; ++e) {
          ...
          float a_value = mixed_ba[step + num_v_heads + k_head_id * kv_ratio + e];
          b_value = act_sigmoid(b_value);
          b_out[(k_head_id * kv_ratio + e) * num_virtual_tokens + out_token_id] = b_value;
          a_out[(k_head_id * kv_ratio + e) * num_virtual_tokens + out_token_id] = a_value;
        }
      } else {
        int step = (lookup(token_id) * num_v_heads + k_head_id * num_v_heads / num_k_heads) * 2;
#pragma unroll
        for (int e = 0; e < kv_ratio; ++e) {
...
#pragma unroll
    for (int e = 0; e < load_size; ++e) {
      ...
      int mixed_z_id = z_src_token * num_k_heads * qkvz_dim + k_head_id * qkvz_dim + qkv_dim + z_dim_id;
      if constexpr (ReorderInput) {
        mixed_z_id = z_src_token * num_k_heads * qkvz_dim + 2 * num_k_heads * head_k_dim + num_v_heads * head_v_dim +
                     k_head_id * z_dim + z_dim_id;
      }
      z_out[z_src_token * num_k_heads * z_dim + k_head_id * z_dim + z_dim_id] = mixed_qkvz[mixed_z_id];
    }
  }
...
void chunk_causal_conv1d(
    ...
    const torch::Tensor& mixed_qkvz,                        // [total_seqlen, num_k_heads * (2 * head_k_dim + 2 *
...
#define SPLIT_DISPATCH(scalar_t, width, reorder_input) \
  ...
  if (mixed_qkvz.scalar_type() == at::kBFloat16) {
    ...
    TORCH_CHECK(
        false, "GDN chunk_causal_conv1d: input dtype must be float16/bfloat16, but got ", mixed_qkvz.scalar_type());
  }
