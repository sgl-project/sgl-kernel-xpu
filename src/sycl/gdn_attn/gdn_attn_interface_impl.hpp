...
SGL_KERNEL_EXPORT void gdn_attention(
    ...
    const torch::Tensor& projected_states_qkvz,  // [num_actual_tokens, num_k_heads / tp_size *
                                                 // (2 * head_k_dim + 2 * head_v_dim *
                                                 // num_v_heads / num_k_heads)]
    const torch::Tensor& projected_states_ba,    // [num_actual_tokens, num_k_heads / tp_size * (2
                                                 // * num_v_heads / num_k_heads)]
    const int64_t num_k_heads,
    const int64_t num_v_heads,
    const int64_t head_k_dim,
    const int64_t head_v_dim,
    torch::Tensor& conv_state,                      // [cache_batch_size, width - 1, num_k_heads / tp_size * (2
    ...
    const torch::Tensor& conv_weights,              // [num_k_heads / tp_size * (2 * head_k_dim + head_v_dim
                                                    // * num_v_heads / num_k_heads), width]
    const std::optional<torch::Tensor>& conv_bias,  // [num_k_heads / tp_size * (2 * head_k_dim + head_v_dim *
    ...
    const std::optional<torch::Tensor>& spec_state_indices_tensor,      // [num_spec_decodes, num_speculative_tokens
                                                                        // + 1]
    const std::optional<torch::Tensor>& num_accepted_tokens,            // [num_spec_decodes]
    const int64_t num_actual_tokens,
    const int64_t tp_size,
    const bool reorder_input,
    const std::optional<torch::Tensor>& workspace) {  // Optional pre-allocated scratch buffer. See below for details.
  ...
  if (num_prefills + num_decodes > 0) {
    if (has_initial_state.has_value()) {
      TORCH_CHECK(has_initial_state->is_contiguous(), "has_initial_state must be contiguous");
      TORCH_CHECK(has_initial_state->dtype() == torch::kBool, "has_initial_state must be of bool dtype");
      TORCH_CHECK(has_initial_state->dim() == 1, "has_initial_state must be 1D of shape [num_prefills + num_decodes]");
      TORCH_CHECK(
    ...
    if (non_spec_token_indx.has_value()) {
      TORCH_CHECK(non_spec_token_indx->is_contiguous(), "non_spec_token_indx must be contiguous");
      TORCH_CHECK(non_spec_token_indx->dtype() == torch::kInt32, "non_spec_token_indx must be of int32 dtype");
      TORCH_CHECK(non_spec_token_indx->dim() == 1, "non_spec_token_indx must be 1D of shape [non_spec_token]");
  ...
  if (num_spec_decodes > 0) {
    ...
    TORCH_CHECK(spec_token_indx->is_contiguous(), "spec_token_indx must be contiguous");
    TORCH_CHECK(spec_token_indx->dtype() == torch::kInt32, "spec_token_indx must be of int32 dtype");
    TORCH_CHECK(spec_token_indx->dim() == 1, "spec_token_indx must be 1D of shape [spec_token]");
    spec_token = spec_token_indx->size(0);

    TORCH_CHECK(spec_state_indices_tensor->is_contiguous(), "spec_state_indices_tensor must be contiguous");
    TORCH_CHECK(
        spec_state_indices_tensor->dtype() == torch::kInt32, "spec_state_indices_tensor must be of int32 dtype");
    TORCH_CHECK(
    ...
    TORCH_CHECK(num_accepted_tokens->is_contiguous(), "num_accepted_tokens must be contiguous");
    TORCH_CHECK(num_accepted_tokens->dtype() == torch::kInt32, "num_accepted_tokens must be of int32 dtype");
    TORCH_CHECK(num_accepted_tokens->dim() == 1, "num_accepted_tokens must be 1D of shape [num_spec_decodes]");
    TORCH_CHECK(num_accepted_tokens->size(0) == num_spec_decodes, "num_accepted_tokens size must be num_spec_decodes");
  }
  ...
  TORCH_CHECK(
      ...
      num_k_heads / tp_size * (2 * head_k_dim + 2 * head_v_dim * num_v_heads / num_k_heads));
  ...
  // When provided, it is assumed to be a single flat 1D torch::kUInt8 tensor.
  // Internal scratch tensors (q/k/v/b/a for decode, or q/k/v/b_prefill/a_prefill
  // for prefill/chunk) are carved out of it by this function as tightly-packed,
  // aligned byte sub-ranges computed from their own shapes/dtypes -- callers do
  // not need to know the internal layout, only the total number of bytes required
  ...
  auto make_ws_tensor = [&](const std::vector<int64_t>& shape, torch::ScalarType st, bool zero_init) -> torch::Tensor {
    int64_t numel = 1;
    for (auto d : shape)
      numel *= d;
    auto opts = torch::dtype(st).device(device).requires_grad(false);
    if (ws_active) {
      int64_t nbytes = numel * static_cast<int64_t>(c10::elementSize(st));
      int64_t aligned_offset = gdn::ws_align_up(ws_cursor_bytes);
      if (aligned_offset + nbytes <= ws_capacity_bytes) {
        torch::Tensor t = torch::from_blob(ws_base + aligned_offset, shape, opts);
        ws_cursor_bytes = aligned_offset + nbytes;
        if (zero_init) t.zero_();
        return t;
      }
      // Once a section fails to fit, stop trying to use the workspace for the
      // remaining sections in this call too, to avoid a partially-workspace
      // -backed, hard-to-reason-about layout.
      ws_active = false;
    }
  ...
  if (spec_token > 0) {
    torch::Tensor q = torch::empty(
        {spec_token, num_k_heads / tp_size, head_k_dim}, torch::dtype(dtype).device(device).requires_grad(false));
    torch::Tensor k = torch::empty(
        {spec_token, num_k_heads / tp_size, head_k_dim}, torch::dtype(dtype).device(device).requires_grad(false));
    torch::Tensor v = torch::empty(
        {spec_token, num_v_heads / tp_size, head_v_dim}, torch::dtype(dtype).device(device).requires_grad(false));
    torch::Tensor b =
        torch::empty({spec_token, num_v_heads / tp_size}, torch::dtype(dtype).device(device).requires_grad(false));
    torch::Tensor a =
        torch::empty({spec_token, num_v_heads / tp_size}, torch::dtype(dtype).device(device).requires_grad(false));
...
#define NATIVE_LAUNCHER                                                                                \
  do {                                                                                                 \
    ...
    auto ws_sections = gdn::gdn_workspace_sections(                                                    \
        /*num_prefills=*/0,                                                                            \
        non_spec_token,                                                                                \
        /*batch_size=*/0,                                                                              \
        num_k_heads,                                                                                   \
        ...
        tp_size,                                                                                       \
        dtype_st);                                                                                     \
    torch::Tensor q = make_ws_tensor(ws_sections[0].shape, ws_sections[0].dtype, /*zero_init=*/false); \
    torch::Tensor k = make_ws_tensor(ws_sections[1].shape, ws_sections[1].dtype, /*zero_init=*/false); \
    torch::Tensor v = make_ws_tensor(ws_sections[2].shape, ws_sections[2].dtype, /*zero_init=*/false); \
    torch::Tensor b = make_ws_tensor(ws_sections[3].shape, ws_sections[3].dtype, /*zero_init=*/false); \
    torch::Tensor a = make_ws_tensor(ws_sections[4].shape, ws_sections[4].dtype, /*zero_init=*/false); \
    gdn::causal_conv1d(                                                                                \
        queue,                                                                                         \
        q,                                                                                             \
        k,                                                                                             \
    ...
    if (num_prefills > 0) {
      ...
      auto ws_sections = gdn::gdn_workspace_sections(
          ...
          tp_size,
          dtype_st);
      torch::Tensor q = make_ws_tensor(ws_sections[0].shape, ws_sections[0].dtype, /*zero_init=*/true);
      torch::Tensor k = make_ws_tensor(ws_sections[1].shape, ws_sections[1].dtype, /*zero_init=*/true);
      torch::Tensor v = make_ws_tensor(ws_sections[2].shape, ws_sections[2].dtype, /*zero_init=*/true);
      torch::Tensor b = make_ws_tensor(ws_sections[3].shape, ws_sections[3].dtype, /*zero_init=*/true);
      torch::Tensor a = make_ws_tensor(ws_sections[4].shape, ws_sections[4].dtype, /*zero_init=*/true);
      ...
      const bool fuse_l2norm = use_tiled ? (2 * head_k_dim <= tiled_feats_per_wg) : true;

      if (use_tiled) {
        gdn::chunk_causal_conv1d_tiled(
            queue,
            q,
            k,
            v,
...
#undef NATIVE_LAUNCHER
  ...
  ::sglkernel::report_kernel_perf("gdn_attention", queue, timer, bytes, flops);
#endif
}
