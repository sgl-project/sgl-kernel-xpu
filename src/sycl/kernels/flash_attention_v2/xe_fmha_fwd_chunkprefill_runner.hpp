 ...
 public:
  ...
  static int q_capacity(ProblemShape const& shape) {
    if constexpr (is_var_len) {
      return shape.seq_len_qo.total_length > 0 ? shape.seq_len_qo.total_length
                                               : shape.batch * shape.seq_len_qo.max_length;
    } else {
  ...
  static Params to_underlying_arguments(Arguments const& args, void* workspace) {
    ...
    size_t oaccum_count =
        size_t(q_tokens) * args.kernel.shape.num_heads_q * args.num_kv_splits * args.kernel.shape.head_size_vo;
...
struct ChunkPrefillSplitKVConfig {
  ...
  static void run(const Arguments& params) {
    ...
    using CollectiveEpilogue = cutlass::fmha::collective::
        FMHAFwdEpilogue<CollectiveMainloop, TileShapeOutput, TensorO, GmemTiledCopyO, TensorLSE>;
    using FMHAKernel = cutlass::fmha::kernel::
        XeFMHAChunkPrefillSplitKVKernel<ProblemShapeType, CollectiveMainloop, CollectiveEpilogue, Scheduler>;
