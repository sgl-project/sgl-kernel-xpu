...
struct causal_conv1d_kernel {
 public:
  ...
  static inline sycl::nd_range<2> get_nd_range(const int num_virtual_tokens, const int qkvz_elems) {
  ...
  [[sycl::reqd_sub_group_size(sub_group_size)]] void operator()(sycl::nd_item<2> item) const {
    ...
    const int global_token_id = (token_indx != nullptr) ? token_indx[token_id] : token_id;
...
#pragma unroll
        for (int e = 0; e < elems_per_item; ++e) {
          b_out[step_local + qkvz_elems_id + e] = mixed_ba[step_global * 2 + qkvz_elems_id + e];
          a_out[step_local + qkvz_elems_id + e] = mixed_ba[step_global * 2 + num_v_heads + qkvz_dim_id + e];
        }
      }
    } else {
      if (qkvz_dim_id < (num_v_heads / num_k_heads)) {
        int step_local = token_id * num_v_heads + k_heads_id * num_v_heads / num_k_heads;
        int step_global = global_token_id * num_v_heads + k_heads_id * num_v_heads / num_k_heads;
        const int ba_elems_per_item = sycl::min(elems_per_item, num_v_heads / num_k_heads);
#pragma unroll
        for (int e = 0; e < ba_elems_per_item; ++e) {
          b_out[step_local + qkvz_dim_id + e] = mixed_ba[step_global * 2 + qkvz_dim_id + e];
          a_out[step_local + qkvz_dim_id + e] = mixed_ba[step_global * 2 + num_v_heads / num_k_heads + qkvz_dim_id + e];
        }
      }
    }
    ...
    if (qkvz_dim_id < q_dim) {
      ...
      is_k = true;
      if constexpr (ReorderInput) {
        mixed_qkvz_id = num_k_heads * head_k_dim + k_heads_id * k_dim + qkvz_dim_id - (q_dim);
      }
    } else if (qkvz_dim_id < q_dim + k_dim + v_dim) {
      is_v = true;
      if constexpr (ReorderInput) {
        mixed_qkvz_id = 2 * num_k_heads * head_k_dim + k_heads_id * v_dim + qkvz_dim_id - (q_dim + k_dim);
      }
    } else {
      is_z = true;
      if constexpr (ReorderInput) {
        mixed_qkvz_id = 2 * num_k_heads * head_k_dim + num_v_heads * head_v_dim + k_heads_id * z_dim + qkvz_dim_id -
                        (q_dim + k_dim + v_dim);
      }
    }
...
#pragma unroll
    for (int i = 0; i < input_load_len; ++i) {
      ...
      const int load_global = (token_indx != nullptr) ? token_indx[load_local] : load_local;
#pragma unroll
      for (int e = 0; e < elems_per_item; ++e) {
        local_input[Width * e + states_load_len + i] = mixed_qkvz[load_global * qkvz_elems + mixed_qkvz_id + e];
      }
    }
...
#pragma unroll
          for (int e = 0; e < elems_per_item; ++e) {
            conv_states_tmp[batch_id * (Width - 1) * conv_elems + i * conv_elems + reordered_elems_id + e] =
...
struct update_states_kernel {
 public:
  ...
  static inline sycl::nd_range<3> get_nd_range(const int batch_size, const int width, const int conv_elems) {
...
struct causal_conv1d_spec_kernel {
 public:
  ...
  static inline sycl::nd_range<2> get_nd_range(const int num_spec_decodes, const int qkvz_elems) {
    const int groups_per_token = (qkvz_elems + elems_per_group - 1) / elems_per_group;
    sycl::range<2> local(1, group_size);
...
#pragma unroll
        ...
        if (qkvz_dim_id < (num_v_heads / num_k_heads)) {
          int step_local = token_id_local * num_v_heads + k_heads_id * num_v_heads / num_k_heads;
          int step_global = global_t * num_v_heads + k_heads_id * num_v_heads / num_k_heads;
          const int ba_elems_per_item = sycl::min(elems_per_item, num_v_heads / num_k_heads);
#pragma unroll
          for (int e = 0; e < ba_elems_per_item; ++e) {
            b_out[step_local + qkvz_dim_id + e] = mixed_ba[step_global * 2 + qkvz_dim_id + e];
            a_out[step_local + qkvz_dim_id + e] =
                mixed_ba[step_global * 2 + num_v_heads / num_k_heads + qkvz_dim_id + e];
          }
        }
      }
    }
...
#pragma unroll
    ...
    const int init_state_id = cache_indices[batch_id * cache_indices_stride_0 + init_col];
...
#pragma unroll
    ...
    if (Width > 1 && init_state_id != pad_slot_id) {
      const T* init_state_ptr = conv_states + init_state_id * conv_states_stride_0;
#pragma unroll
      for (int i = 0; i < Width - 1; ++i) {
#pragma unroll
        for (int e = 0; e < elems_per_item; ++e) {
...
#pragma unroll
      for (int e = 0; e < elems_per_item; ++e) {
        local_input[Width * e + Width - 1] = mixed_qkvz[global_t * qkvz_elems + mixed_qkvz_id + e];
      }
...
#pragma unroll
        for (int e = 0; e < elems_per_item; ++e) {
          q_out[token_id_local * num_k_heads * q_dim + k_heads_id * q_dim + qkvz_dim_id + e] = res[e];
        }
      } else if (is_k) {
#pragma unroll
        for (int e = 0; e < elems_per_item; ++e) {
          k_out[token_id_local * num_k_heads * k_dim + k_heads_id * k_dim + qkvz_dim_id - q_dim + e] = res[e];
        }
      } else {  // is_v
#pragma unroll
        for (int e = 0; e < elems_per_item; ++e) {
          v_out[token_id_local * num_k_heads * v_dim + k_heads_id * v_dim + qkvz_dim_id - (q_dim + k_dim) + e] = res[e];
        }
      }
      ...
      if (Width > 1) {
        const int save_state_id = cache_indices[batch_id * cache_indices_stride_0 + t_local];
        if (save_state_id != pad_slot_id) {
          T* save_state_ptr = conv_states + save_state_id * conv_states_stride_0;
#pragma unroll
          for (int i = 0; i < Width - 1; ++i) {
#pragma unroll
            for (int e = 0; e < elems_per_item; ++e) {
...
void causal_conv1d(
    ...
    const torch::Tensor& mixed_qkvz,                        // [num_actual_tokens, num_k_heads * (2 * head_k_dim + 2 *
                                                            // head_v_dim * num_v_heads / num_k_heads)]
    const torch::Tensor& mixed_ba,                          // [num_actual_tokens, num_k_heads * (2 *
                                                            // num_v_heads / num_k_heads)]
    const torch::Tensor& conv_weights,                      // [num_k_heads * (2 * head_k_dim + head_v_dim *
                                                            // num_v_heads / num_k_heads), width]
    const std::optional<torch::Tensor>& conv_bias,          // [num_k_heads * (2 * head_k_dim + head_v_dim * num_v_heads
                                                            // / num_k_heads)] or None
    torch::Tensor& conv_states,                             // [cache_batch_size, width - 1, num_k_heads * (2 *
  ...
  torch::Tensor conv_states_tmp = torch::empty(
      {std::max(batch_size, 1), width - 1, conv_elems}, torch::dtype(dtype).device(device).requires_grad(false));
...
#define KERNEL_LAUNCHER(scalar_t, width, reorder_input)                                                    \
  kernel_launcher<scalar_t, width, reorder_input>(                                                         \
      ...
      num_accepted_tokens.has_value() ? reinterpret_cast<int*>(num_accepted_tokens->data_ptr()) : nullptr, \
      act_mode,                                                                                            \
      pad_slot_id,                                                                                         \
      batch_size,                                                                                          \
      num_virtual_tokens,                                                                                  \
