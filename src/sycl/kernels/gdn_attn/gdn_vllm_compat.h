...
#define CHECK_STRIDE_ALIGNMENT(x)                                                                                    \
  do {                                                                                                               \
    ...
    TORCH_CHECK((x).stride(-1) == 1, #x " expects the last dim to be contiguous (stride==1), got ", (x).stride(-1)); \
    TORCH_CHECK(                                                                                                     \
        _last* _es % 16 == 0,                                                                                        \
        #x " last dim size=",                                                                                        \
        _last,                                                                                                       \
...
static inline bool is_pvc(at::DeviceIndex device_index = -1) {
  return get_device_architecture(device_index) == syclex::architecture::intel_gpu_pvc;
}

static inline bool is_xe2_arch(at::DeviceIndex device_index = -1) {
  auto arch = get_device_architecture(device_index);
  return arch == syclex::architecture::intel_gpu_bmg_g21 || arch == syclex::architecture::intel_gpu_bmg_g31 ||
         arch == syclex::architecture::intel_gpu_lnl_m || arch == syclex::architecture::intel_gpu_pvc;
}

static inline bool is_xe3_arch(at::DeviceIndex device_index = -1) {
  auto arch = get_device_architecture(device_index);
  return arch == syclex::architecture::intel_gpu_ptl_h || arch == syclex::architecture::intel_gpu_ptl_u ||
...
static inline bool force_xe_default_kernel() {
  ...
  if (env_val.has_value()) {
    return env_val.value() == "1" || env_val.value() == "true" || env_val.value() == "TRUE";
  }
  return false;
}
...
struct AccumulateType {
 private:
  static constexpr bool is_narrow_float = std::is_same_v<T, at::Half> || std::is_same_v<T, at::BFloat16> ||
                                          std::is_same_v<T, c10::Float8_e4m3fn> || std::is_same_v<T, c10::Float8_e5m2>;

  static constexpr bool is_integer = std::is_same_v<T, int8_t> || std::is_same_v<T, uint8_t> ||
                                     std::is_same_v<T, char> || std::is_same_v<T, int16_t> ||
                                     std::is_same_v<T, int32_t> || std::is_same_v<T, int64_t>;

  static constexpr bool is_complex = std::is_same_v<T, c10::complex<float>> || std::is_same_v<T, c10::complex<double>>;
 ...
 public:
  using type = std::conditional_t<
      ...
      std::conditional_t<
          ...
          T,
          std::conditional_t<is_integer, int64_t, std::conditional_t<is_complex, T, T>>>>;
};
