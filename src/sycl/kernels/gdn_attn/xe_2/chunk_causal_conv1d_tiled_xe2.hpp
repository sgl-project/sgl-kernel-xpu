...
struct chunk_causal_conv1d_tiled_kernel {
 public:
  ...
  [[sycl::reqd_sub_group_size(sub_group_size)]] void operator()(sycl::nd_item<2> item) const {
    ...
    T* slm_input = reinterpret_cast<T*>(reinterpret_cast<char*>(slm_data) + slm_meta_bytes);
...
#pragma unroll
          for (int e = 0; e < elems_per_item; ++e) {
            vals[e] = mixed_qkvz[global_tok * qkvz_elems + global_feat_offset + e];
          }
        } else {
#pragma unroll
          for (int e = 0; e < elems_per_item; ++e) {
...
#pragma unroll
        for (int e = 0; e < elems_per_item; ++e) {
          res[e] += static_cast<float>(slm_input[slot * feats_per_wg + local_feat + e]) *
...
#pragma unroll
      ...
      if (fuse_l2norm && feat_chunk_id == 0) {
        ...
        float* norm_slm =
            reinterpret_cast<float*>(reinterpret_cast<char*>(slm_data) + slm_meta_bytes + slm_data_elems * sizeof(T));
...
#pragma unroll
        for (int e = 0; e < elems_per_item; ++e) {
          conv_states_tmp[batch_id * (Width - 1) * conv_elems + i * conv_elems + reordered_feat + e] =
