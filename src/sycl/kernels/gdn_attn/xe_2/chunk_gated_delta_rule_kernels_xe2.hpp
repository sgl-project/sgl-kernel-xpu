...
void kernel_launcher(
  ...
  if (vllm::xpu::is_bmg()) {
    ...
    using MMAInverse =
        typename TiledMMAHelper<MMA_Atom<decltype(op)>, Layout<WGTileInverse>, SGLayoutInverse>::TiledMMA;
    ...
    sycl::range<3> global_inverse(
        1,
        (sm_count * MaxThreadsPerSM / MaxThreadsPerWorkgroupInverse + num_v_heads - 1) / num_v_heads * num_v_heads,
        1);
    ...
    sycl::range<3> global_inverse(1, sm_count * MaxThreadsPerSM / inverse_items, 1);
    int slm_size_inverse = chunk_size * chunk_size * 2;

    queue.submit([&](sycl::handler& cgh) {
      sycl::local_accessor<float, 1> local_mem(sycl::range<1>(slm_size_inverse), cgh);
