...
l2norm_kernel(const T* q, const T* k, const int total_virtual_seqlen, const int num_k_heads, const int head_k_dim) {
  ...
  for (int k_dim_idx = sg_local_id * l2norm_elem_per_item; k_dim_idx < head_k_dim;
       k_dim_idx += l2norm_sub_group_size * l2norm_elem_per_item) {
    for (int e = 0; e < l2norm_elem_per_item; ++e) {
      q_ptr[k_dim_idx + e] = static_cast<T>(static_cast<float>(q_ptr[k_dim_idx + e]) * q_inv);
      k_ptr[k_dim_idx + e] = static_cast<T>(static_cast<float>(k_ptr[k_dim_idx + e]) * k_inv);
    }
  }
}
...
SYCL_EXTERNAL void l2norm_vectorized_kernel(
  ...
  float q_sum = 0.0f;
  float k_sum = 0.0f;
  for (int vec_idx = sg_local_id; vec_idx < num_vec; vec_idx += l2norm_sub_group_size) {
...
void l2norm_launch(
  ...
  static constexpr int MaxSubgroupsPerWorkgroup = MaxThreadsPerSM / l2norm_sub_group_size;
  const int total_qk_heads = total_virtual_seqlen * num_k_heads;
  const int wg_count = (total_qk_heads + MaxSubgroupsPerWorkgroup - 1) / MaxSubgroupsPerWorkgroup;
