...
// Enable Intel Xe (BMG) code paths in cutlass/cute (sgl convention, matches other SYCL TUs).
...
class DensePrefillFwdKernel {
 public:
  ...
  using FragA =
      expand_sg_fragment_t<SingleFragA, 1, Traits::D_V_PER_SPLIT / get<1>(TileShapePV{})>;  // (atom val,q',v',VV)
  ...
  void operator()(const Params& params, char* smem_buf) const {
    ...
    auto gathered_k_layout =
        make_layout(make_shape(params.topk, D_QK), make_stride(params.stride_gathered_k_topk, _1{}));
    auto gathered_v_layout =
        make_layout(make_shape(Traits::D_V_PER_SPLIT, params.topk), make_stride(_1{}, params.stride_gathered_k_topk));
    ...
    // auto tiled_copy_O = (ReduceK{} > _1{}) ? make_block_2d_copy_D_subtiled(mma_pv, ReduceFragA{}.tv_layout(),
    ...
    auto mask_rS = [&](int block_idx) {
      ...
      for (int i = 0; i < tSrS.size(); ++i) {
        ...
        if (!is_valid) {
          // note that cannot use -INFINITY due to numerical issue when applying exp2(prev_max-cur_max) later, use the
    ...
    auto reduce_L = [&]() {
      ...
      if constexpr (ReduceK{} == _1{}) {
        ...
        auto sA =
            make_tensor(make_smem_ptr<ElementA>(&shared_storage.a_data), sA_layout);  // (q,v,rblk_dst,rblk_src,a_tile)
        auto sA_max = make_tensor(
            make_smem_ptr<ElementA>(&shared_storage.a_max_data), sA_row_layout);  // (q,rblk_dst,rblk_src,a_tile)
        auto sA_sum = make_tensor(
            make_smem_ptr<ElementA>(&shared_storage.a_sum_data), sA_row_layout);  // (q,rblk_dst,rblk_src,a_tile)
        ...
        if constexpr (ReduceK{} > 1) {
          ...
          for (int i = 0; i < rA.size(); ++i) {
            ...
            if (head_idx < params.h_q) {
              ...
              ElementA sink_exp_sum =
                  sycl::native::exp2(static_cast<ElementA>(attn_sink_val * FLASH_NAMESPACE::LOG_2_E) - global_max);
          ...
          if (tid_in_sg < valid_tid_per_sg && sg_id * valid_tid_per_sg < Traits::B_H) {
            ...
            if (head_idx < params.h_q) {
              ...
              for (int i = 0; i < rA_sum.size(); ++i) {
                rA_sum(i) +=
                    sycl::native::exp2(static_cast<ElementA>(attn_sink_val * FLASH_NAMESPACE::LOG_2_E) - rA_max(i));
              }
            }
          }
...
class SparsePrefillGatherKernel {
 public:
  ...
  static_assert(D_QK % (SUBGROUP_SIZE * ELEMENTS_PER_LANE) == 0, "D_QK must be divisible by one subgroup span");
      ...
      for (int d_base = lane_id * ELEMENTS_PER_LANE; d_base < D_QK; d_base += SUBGROUP_SIZE * ELEMENTS_PER_LANE) {
        ...
        if (can_pack) {
          const PackedElementKV value =
              valid_token ? *reinterpret_cast<const PackedElementKV*>(params.kv + kv_row_offset + d_base)
                          : PackedElementKV(0);
...
void launch_sparse_mla_prefill_gather_kernel(const XPUSparseAttnFwdParams& params) {
  ...
  // gather kernel uses normal GRF mode for more parallel subgroups
...
#if defined(XPU_ENABLED_XE3P)
      sycl::ext::intel::experimental::grf_size<512>
#else
      sycl::ext::intel::experimental::grf_size<256>
#endif
  };
...
void launch_sparse_mla_prefill_fwd_kernel(const XPUSparseAttnFwdParams& params) {
  ...
  for (int chunk_start = 0; chunk_start < total_s_q; chunk_start += chunk_size) {
    const int cur_chunk = std::min(chunk_size, total_s_q - chunk_start);

    // Create a chunk-local copy of params with adjusted s_q and pointers
    XPUSparseAttnFwdParams chunk_params = params;
