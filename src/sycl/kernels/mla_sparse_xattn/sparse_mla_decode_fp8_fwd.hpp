...
class SparseDecodeGatherDequantKernel {
 public:
  ...
  void operator()(const Params& params, char* smem_buf) const {
    ...
    const int main_topk_length = resolve_topk_length(params.topk_length, params.topk, params.stride_topk_length_b);
...
template <int D_QK, bool IS_FP8_QUERY, bool HAS_ATTN_SINK, typename Traits>
 ...
 public:
  ...
  void operator()(const Params& params, char* smem_buf) const {
    ...
    auto gathered_k_layout =
        make_layout(make_shape(params.gathered_topk, D_QK), make_stride(params.stride_gathered_k_topk, _1{}));
    auto gathered_v_layout = make_layout(
        make_shape(Traits::D_V_PER_SPLIT, params.gathered_topk), make_stride(_1{}, params.stride_gathered_k_topk));
...
void launch_sparse_mla_decode_fp8_fwd_kernel_policy(const XPUSparseDecodeAttnFwdParams& params) {
  ...
  if (params.gathered_topk > 0) {
    ...
    auto event = compat::experimental::launch<cutlass::device_kernel<GatherDequantKernel>, GatherDequantKernel>(
...

  {
    ...
    compat::experimental::launch_policy policy{sycl_grid, sycl_block, launch_props, kernel_props};
    auto event = compat::experimental::launch<cutlass::device_kernel<DenseKernel>, DenseKernel>(
        policy, params.queue, static_cast<SparseAttnDecodeParams>(params));
  }
}
