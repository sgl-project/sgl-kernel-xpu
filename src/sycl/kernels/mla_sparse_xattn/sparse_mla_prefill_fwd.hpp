...
void launch_sparse_mla_prefill_fwd_kernel(const XPUSparseAttnFwdParams& params) {
    ...
    for (int chunk_start = 0; chunk_start < total_s_q; chunk_start += chunk_size) {
        ...
        chunk_params.topk_length = (HAVE_TOPK_LENGTH && params.topk_length != nullptr)
            ? params.topk_length + chunk_start : params.topk_length;
        chunk_params.out = params.out + static_cast<int64_t>(chunk_start) * params.h_q * params.d_v;
        chunk_params.max_logits = params.max_logits != nullptr
            ? params.max_logits + static_cast<int64_t>(chunk_start) * params.h_q : nullptr;
        chunk_params.lse = params.lse != nullptr
            ? params.lse + static_cast<int64_t>(chunk_start) * params.h_q : nullptr;
