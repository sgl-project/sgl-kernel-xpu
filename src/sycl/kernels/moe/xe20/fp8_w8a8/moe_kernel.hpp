...
class MoEGEMMFp8W8A8 {
 public:
  ...
  struct Params {
    ...
    bool act_scale_grouped = false;
    int32_t act_scale_k_groups = 1;
  };
  ...
  auto make_Bias_tensors(float* ptr_Bias, int N) {
    if constexpr (FuseAct) {
      if constexpr (ActType == SWIGLU_GPT_OSS) {
        ...
        auto Bias1 = make_tensor(make_gmem_ptr<float>(ptr_Bias + 1), make_layout(make_shape(N / 2), make_stride(_2{})));
      ...
      while (group_m_id < cumsum_tiles_for_experts) {
        ...
        if constexpr (FuseAct) {
          ...
          mainloop(
              ...
              params.gemm1_alpha,
              params.gemm1_limit,
              params.act_scale_grouped,
              params.act_scale_k_groups);
        } else {
