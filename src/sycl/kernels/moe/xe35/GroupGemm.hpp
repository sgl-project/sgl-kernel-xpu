/**
 * Xe35-specific grouped GEMM dispatch.
 *
 * Shares the same CuTe-based kernel template as xe20 (bf16/moe_kernel.hpp)
 * but uses tile-selection heuristics tuned for the Xe3 architecture:
 * - 32 Xe cores (vs 64 on Xe2)
 * - Higher per-core throughput
 * - Different occupancy/wave-quantization tradeoffs
 */
...
// NOTE: uses a Xe3-dedicated copy of the bf16 kernel template (see
// kernels/moe/xe35/bf16/moe_kernel.hpp), not xe20's, because the two repos'
// Xe2 kernel headers have drifted; keeping Xe3 self-contained avoids
// perturbing the existing Xe2 (BMG) bf16 grouped GEMM.
...
// Xe35-tuned tile selection.
// Xe3 has 32 wider cores (vs 64 on Xe2) with higher per-core throughput.
// Key differences from xe20 dispatch, validated on CRI device-clock:
//   - Unfused: 128x128 tile for avg_m 9..128 (xe20 uses smaller 16x64/32x64
//     tiles at low avg_m, but on xe35 the wider cores benefit from the larger
//     tile's better arithmetic intensity — measured +5% at avg_m=32).
//   - At avg_m >= 64 with moderate K (<=1024), prefer 256x256 for better
//     N-coverage (e.g. MoE down-projection N=2048, K=768).
//   - Fused paths unchanged from xe20 (128x64 / 256x64).
SGL_KERNEL_EXPORT void moe_grouped_mm_nt_xe35(
    torch::Tensor& output,
  ...
  if (bias.has_value()) {
    TORCH_CHECK(
        ...
        "moe_grouped_mm_nt_xe35: bias must be float32 (at::kFloat) to match kernel expectations");
    TORCH_CHECK(
...
#undef DISPATCH_MOE
#undef DISPATCH_MOE_HELPER_ACT_TYPE
#undef DISPATCH_MOE_HELPER_FUSE_ACT
#undef DISPATCH_MOE_HELPER_BIAS
#undef LAUNCH_MOE
