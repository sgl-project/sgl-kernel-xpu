...
import pytest
import torch
from sgl_kernel import fused_q_norm_rope
...
def _ref_rope_interleaved(
    ...
    for b in range(B):
        ...
        freq = freqs_cis[
            pos
        ]  # (rope_dim // 2,) complex — becomes interleaved [re0, im0, re1, im1, ...] via view_as_real
...
def test_fused_q_norm_rope_correctness(batch_size, num_heads, head_dim):
    ...
    fused_q_norm_rope(q_input, q_output, freqs_real, positions, eps)
...
def test_fused_q_norm_rope_zero_batch():
    ...
    fused_q_norm_rope(q_input, q_output, freqs_real, positions, 1e-6)
