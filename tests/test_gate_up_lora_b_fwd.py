...
def _zero_weight_rank_tail(
    ...
    For LoRA-B the rank axis is the reduction (K) dimension -- ``gate_up_lora_b`` is
    ``[num_loras, 2*N, max_rank]`` -- so an adapter of rank ``R_l`` only activates
    the first ``R_l`` columns. The kernel computes the full ``max_rank`` reduction
    unconditionally and trusts that the tail columns are zero; this helper enforces
    that contract for test inputs so the reference and kernel outputs agree (and
    makes the rank == 0 case a genuine all-zero LoRA term).
    """
    out = weights.clone()
...
def _reference_gate_up_lora_b_fwd(
    ...
    Implements, per segment ``s`` (adapter ``l``) and projection ``p in {gate,up}``::

        out[i, band_p] = scalings[l] * (input_x[i, band_p] @ gate_up_lora_b[l, band_p]^T)
                         + base_output[i, band_p]

    where ``band_p`` is the input K-band ``[p*K, (p+1)*K)`` and the output/weight
    band ``[p*N, (p+1)*N)``; the residual term is only added when ``base_output`` is
    supplied.
    """
    ...
    n_total = gate_up_lora_b.size(1)  # 2 * N
    max_rank = gate_up_lora_b.size(2)
    n = output_dim
    ...
    for s in range(num_segments):
        ...
        for p in range(2):
            ...
            w = gate_up_lora_b[lora, p * n : (p + 1) * n, :].float()  # [N, max_rank]
            out[start:end, p * n : (p + 1) * n] += scal[lora] * (in_band @ w.T)
    return out.to(gate_up_lora_b.dtype)
...
def _run_and_compare(
    *,
    ...
    # input_x is the LoRA-A projection packed for gate/up: [num_tokens, 2 * max_rank].
    input_x = torch.randn(num_tokens, 2 * max_rank, dtype=dtype, device="xpu")
    gate_up_lora_b = torch.randn(
        num_loras, n_total, max_rank, dtype=dtype, device="xpu"
    )
    gate_up_lora_b = _zero_weight_rank_tail(gate_up_lora_b, lora_ranks)

    base_output = None
    if use_base_output:
        base_output = torch.randn(num_tokens, n_total, dtype=dtype, device="xpu")

    out = gate_up_lora_b_fwd(
        input_x=input_x,
    ...
    ref = _reference_gate_up_lora_b_fwd(
        input_x,
        gate_up_lora_b,
        output_dim,
        seg_indptr,
...
@pytest.mark.parametrize("output_dim", [64, 4096, 256])
@pytest.mark.parametrize("max_rank", [8, 64])
def test_gate_up_lora_b_fwd_basic_shapes(dtype, output_dim, max_rank):
    num_tokens = 64
    num_loras = 3
...
def test_gate_up_lora_b_fwd_base_output_residual(dtype, use_base_output):
...
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_gate_up_lora_b_fwd_per_segment_scalings(dtype):
...
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_gate_up_lora_b_fwd_single_segment_single_lora(dtype):
    num_tokens = 128
    max_rank = 16
    output_dim = 512
    num_loras = 1
...
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_gate_up_lora_b_fwd_single_token_segments(dtype):
...
def test_gate_up_lora_b_fwd_large_num_tokens_many_segments(dtype, num_tokens):
    max_rank = 16
    output_dim = 512
    num_loras = 4
...
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_gate_up_lora_b_fwd_rank_padding_matches_reference(dtype):
...
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
@pytest.mark.parametrize("use_base_output", [False, True])
def test_gate_up_lora_b_fwd_zero_rank(dtype, use_base_output):
...
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_gate_up_lora_b_fwd_segment_boundaries_precise_routing(dtype):
...
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_gate_up_lora_b_fwd_empty_segments_mixed_in(dtype):
...
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_gate_up_lora_b_fwd_empty_input(dtype):
    ...
    input_x = torch.empty((0, 2 * max_rank), dtype=dtype, device="xpu")
    gate_up_lora_b = torch.randn(
        num_loras, n_total, max_rank, dtype=dtype, device="xpu"
    )
...
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
@pytest.mark.parametrize("use_base_output", [False, True])
def test_gate_up_lora_b_fwd_zero_max_rank(dtype, use_base_output):
    ...
    input_x = torch.empty((num_tokens, 2 * max_rank), dtype=dtype, device="xpu")
...
def test_gate_up_lora_b_fwd_int64_index_tensors_accepted():
    """seg_indptr / weight_indices in int64 should be cast internally."""
    ...
    ref = _reference_gate_up_lora_b_fwd(
        input_x, gate_up_lora_b, output_dim, seg_indptr, weight_indices, scalings
    )
    rtol, atol = _tolerances(dtype)
    torch.testing.assert_close(out, ref, rtol=rtol, atol=atol)
...
def _make_valid_kwargs():
    ...
    return dict(
        input_x=torch.randn(
            num_tokens, 2 * max_rank, dtype=torch.float16, device="xpu"
        ),
        gate_up_lora_b=torch.randn(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("gate_up_lora_b_dim", "gate_up_lora_b must be a 3D tensor"),
        ...
        ("dtype_mismatch", "Input tensor dtype must match gate_up_lora_b dtype"),
        ("base_output_dim", "base_output must be a 2D tensor"),
        ("base_output_shape", "base_output must have shape"),
        ("base_output_dtype", "base_output dtype must match gate_up_lora_b dtype"),
    ],
)
def test_gate_up_lora_b_fwd_input_validation(bad_case, expected_msg):
    kwargs = _make_valid_kwargs()

    if bad_case == "input_x_dim":
        kwargs["input_x"] = kwargs["input_x"].view(-1)
    elif bad_case == "gate_up_lora_b_dim":
        kwargs["gate_up_lora_b"] = kwargs["gate_up_lora_b"].view(1, -1)
    ...
    elif bad_case == "output_dim_mismatch":
        # gate_up_lora_b.size(1) == 16, so output_dim = 7 breaks 2*output_dim == 16.
        kwargs["output_dim"] = 7
    elif bad_case == "input_x_rank_mismatch":
        # input_x.size(1) must equal 2 * max_rank (8).
        kwargs["input_x"] = torch.randn(4, 6, dtype=torch.float16, device="xpu")
        ...
        # max_rank = gate_up_lora_b.size(2) = 4
