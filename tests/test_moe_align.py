...
@pytest.mark.parametrize(
    ...
    list(
        itertools.product(
            ...
            [63, 64, 160, 256, 257, 260, 264],  #  num_experts
...
def test_moe_align_block_size_compare_implementations(
    ...
    assert torch.allclose(expert_ids_xpu, expert_ids_triton, atol=0, rtol=0), (
    ...
    assert torch.allclose(
        num_tokens_post_pad_xpu, num_tokens_post_pad_triton, atol=0, rtol=0
    ), (
    ...
    expert_idx = expert_ids_xpu.max().item()

    # Get the first and last block id where expert_ids_xpu == expert_idx
    matching_indices = torch.where(expert_ids_xpu == expert_idx)[0]
    block_sorted_start = matching_indices[0].item() * block_size
    block_sorted_end = min(
        (matching_indices[-1].item() + 1) * block_size, num_tokens_post_pad_xpu.item()
    )

    selected_sorted_ids_xpu = sorted_ids_xpu[
