...
def test_prepare_input_moe(num_tokens, num_experts, top_k, hidden_dims, dtype):
    ...
    output_tensor_xpu = torch.empty((num_tokens * top_k, hidden_dims), dtype=dtype, device=device)
    scatter_tokens_to_experts(input_tensor_xpu, output_permutation_xpu, output_tensor_xpu)
